 ...
 * absence, for routing.
 */
export function rawProfileExifOrientation(
  text: string,
  maxBytes: number,
  ...
  if (lines.length < 4 || countLine === undefined) {
    throw new PngStructureError(
      "malformed-file",
      "Raw profile text is missing its header.",
    );
  }
  const trimmedCount = countLine.trim();
  const declaredLength = Number.parseInt(trimmedCount, 10);
  if (
    trimmedCount.length === 0 ||
    String(declaredLength) !== trimmedCount ||
    declaredLength < 0
  ) {
    throw new PngStructureError(
      "malformed-file",
      "Raw profile declared length is invalid.",
    );
  }
  if (declaredLength > maxBytes) {
    throw new PngStructureError(
      "unsafe-structure",
      `Raw profile declared length ${declaredLength} exceeds the ${maxBytes}-byte bound.`,
  ...
  if (
    ...
    throw new PngStructureError(
      "malformed-file",
      "Raw profile hex data is missing or contains a non-hex digit.",
    );
  }
  const data = Buffer.from(hexSlice, "hex");
  const found = parseExif(data);
  if (found.orientation.status === "valid") return found.orientation.value;
  if (found.orientation.status === "absent") return undefined;
  return "invalid";
}
