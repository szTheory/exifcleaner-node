...
function collectMetadata(parsed: ParsedWebp): {
...
export async function inspectFile(
  ...
  if (isAborted(signal)) return err(aborted(filePath));
...
function outputSize(plan: readonly OutputChunk[]): number {
  return plan.reduce(
  ...
  while (copied < chunk.size) {
    ...
    while (blockWritten < length) {
      const writeResult = await destination.write(
        ...
        blockWritten,
        length - blockWritten,
        destinationPosition + copied + blockWritten,
      );
...
async function verifyOutput(
  ...
  try {
    ...
    if (preserveOrientation && expectedOrientation !== undefined) {
      ...
      if (
        ...
        return err({
          code: "verification-failed",
          detail:
            "Destination EXIF contains more than the preserved Orientation.",
          path: destinationPath,
        });
      }
    ...
    if (isAborted(signal)) return err(aborted(destinationPath));
    return err({
      code: "verification-failed",
      detail:
...
export async function sanitizeFile(
  ...
  try {
    ...
    const orientation =
      metadata.orientation.status === "valid"
    ...
    if (
      sourceNamespaces.has("EXIF") &&
      !(options.preserveOrientation && orientation !== undefined)
    )
      removedNamespaces.push("EXIF");
    if (sourceNamespaces.has("XMP")) removedNamespaces.push("XMP");
    ...
    return ok({
      ...
      preserved: {
        ...
        colorProfile:
          options.preserveColorProfile && sourceNamespaces.has("ICC"),
