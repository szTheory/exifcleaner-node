...
function isForbiddenRuntimeModule(specifier) {
  ...
  return [...forbiddenRuntimeModules].some(
    (forbidden) => bare === forbidden || bare.startsWith(`${forbidden}/`),
  );
}
...
export async function checkRuntimeSurface(packageRoot) {
  ...
  try {
    for (const file of await javascriptFiles(root)) {
      ...
      if (/\b(?:readdir|glob)\b/u.test(source))
        diagnostics.push(
          diagnostic(display, "runtime directory scan is forbidden"),
        );
      if (/\b(?:npm\s+install|node-gyp|download)\b/iu.test(source))
        diagnostics.push(
          diagnostic(display, "runtime build or download surface is forbidden"),
        );
...
export async function checkPackedListing(files) {
  ...
  if (!Array.isArray(files) || !files.every((file) => typeof file === "string"))
    return [diagnostic("packed listing", "must be an array of paths")];
  const { EXACT_ARTIFACTS } = artifactContract();
  const exactNativePaths = EXACT_ARTIFACTS.map(
  ...
  if (nativePaths.join("\n") !== exactNativePaths.join("\n"))
    diagnostics.push(
      diagnostic(
        "packed listing",
        "packed native path set must contain exactly six literal D-50 paths",
      ),
    );
...
async function reportsFrom(directory) {
  ...
  for (const artifact of EXACT_ARTIFACTS)
    reports[artifact.tuple] = await readFile(
      resolve(directory, `${artifact.tuple}.json`),
      "utf8",
    );
  return reports;
}
...
export async function checkAssembly({
  ...
  if (evidenceScope !== "test-fixture" && evidenceScope !== "final-release")
    return [
      diagnostic("evidence scope", "must be test-fixture or final-release"),
    ];
  try {
    ...
    if (
      ...
      Object.values(reports).some(
        (report) => JSON.parse(report).evidenceScope === "test-fixture",
      )
    )
      return [
        diagnostic(
          "assembly",
          "fixture evidence is rejected for final release admission",
        ),
      ];
    const { EXACT_ARTIFACTS, validateManifest } = artifactContract();
    await validateManifest(resolve(root), manifest, reports);
    const extraNodes = await nativeFilesOutsideExactPaths(
      resolve(root),
      EXACT_ARTIFACTS.map((artifact) => artifact.path),
    );
    if (extraNodes.length > 0)
      return [diagnostic(extraNodes[0], "extra native binary is forbidden")];
    return [];
  } catch (error) {
...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 2) {
    if (!args[index].startsWith("--") || !args[index + 1])
      throw new Error(
        "Usage: runtime_surface_gate.mjs <package-root> | --source <root> | --assembly-root <root> --manifest <file> --reports-dir <dir> --evidence-scope <test-fixture|final-release> | --packed-listing <json>",
      );
...
async function main() {
  ...
  if (args.length === 1 && !args[0].startsWith("--")) {
    ...
    if (diagnostics.length === 0)
      return console.log("Runtime surface gate passed");
  ...
  else if (values["packed-listing"])
    diagnostics = await checkPackedListing(
      JSON.parse(await readFile(resolve(values["packed-listing"]), "utf8")),
    );
  else if (
    values["assembly-root"] &&
    values.manifest &&
    values["reports-dir"] &&
    values["evidence-scope"]
  ) {
    label = "assembly";
    diagnostics = await checkAssembly({
      root: values["assembly-root"],
      manifestPath: values.manifest,
      reportsDirectory: values["reports-dir"],
      evidenceScope: values["evidence-scope"],
    });
