...
export function validateIccForPreservation(payload) {
    ...
    for (let index = 0; index < tagCount; index += 1) {
        ...
        if (!physicalRanges.has(key))
            physicalRanges.set(key, range);
    }
    const ranges = [...physicalRanges.values()].sort((left, right) => left.offset - right.offset);
    let expectedOffset = tableEnd;
    for (const range of ranges) {
        if (range.offset !== expectedOffset)
            return rejected("invalid", "ICC profile tag payloads are not canonical contiguous ranges.");
