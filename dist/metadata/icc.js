...
function ascii(payload, offset, length) {
    return payload
        .toString("ascii", offset, offset + length)
        .replace(/\0+$/u, "");
}
export function parseIcc(payload) {
    const entries = [
    ...
    entries.push({ namespace: "ICC", name: "DeclaredSize", value: declaredSize }, { namespace: "ICC", name: "CMMType", value: ascii(payload, 4, 4) }, {
        namespace: "ICC",
        name: "Version",
        value: `${payload[8] ?? 0}.${((payload[9] ?? 0) >> 4).toString(16)}.${((payload[9] ?? 0) & 0x0f).toString(16)}`,
    }, { namespace: "ICC", name: "DeviceClass", value: ascii(payload, 12, 4) }, { namespace: "ICC", name: "ColorSpace", value: ascii(payload, 16, 4) }, { namespace: "ICC", name: "PCS", value: ascii(payload, 20, 4) }, { namespace: "ICC", name: "Platform", value: ascii(payload, 40, 4) }, { namespace: "ICC", name: "Manufacturer", value: ascii(payload, 48, 4) }, { namespace: "ICC", name: "Model", value: ascii(payload, 52, 4) }, {
        namespace: "ICC",
    ...
    const maximumRepresentable = Math.floor((payload.length - ICC_HEADER_BYTES - ICC_TAG_COUNT_BYTES) /
    ...
    for (let index = 0; index < tagCount; index += 1) {
        ...
        if (dataOffset < ICC_HEADER_BYTES ||
