...
import { metadataPng, screenshotShapedPng } from "../../fixtures.js";
import {
  LOGICAL_OPERATIONS,
  NamedBarrier,
  applyFaultPlan,
...
interface TransactionFixture {
  ...
  readonly admission: PngAdmission;
  readonly plan: PngOutputPlan;
}

async function fixture(
  sourceBytes: Buffer = metadataPng(),
): Promise<TransactionFixture> {
  const directory = await mkdtemp(join(tmpdir(), "exifcleaner-qtx-png-"));
  ...
  const admission = await pngHandler.admit(source, stats.size);
  return {
    directory,
    sourcePath,
    destinationPath,
    ...
    admission,
    plan: pngHandler.buildOutputPlan(admission, false, false, false, undefined),
  };
}
...
async function run(
  ...
  return runSafeTransaction({
    ...
    handler: options.handler ?? pngHandler,
...
function postPublicationCloseFaultFileOps(
  ...
  const nameFor = (handle: Awaited<ReturnType<typeof open>>): string => {
    ...
    if (
      ...
      !path.endsWith(pngHandler.stagingFileName)
    )
...
describe("deterministic transaction qualification (PNG)", () => {
    ...
    async (target: PostPublicationCloseTarget) => {
      ...
      const restore = setNativePublicationBindingForTests({
        ...
        publishNoReplace() {
          ...
          renameSync(
            join(stageDirectoryPath, pngHandler.stagingFileName),
  ...
  it.each(terminalFaults)(
    ...
    async (operation: LogicalOperation) => {
      ...
      const result = await run(prepared, {
        ...
        handler: controller.wrapHandler(pngHandler),
  ...
  it("injects stage-write once with terminal safety on a structurally complex PNG (iDOT, iCCP, cICP, eXIf)", async () => {
    const prepared = await fixture(screenshotShapedPng());
    const controller = applyFaultPlan(NODE_FILE_OPS, {
    ...
    expect(controller.evidence().writerAttempts).toBe(1);
    expect(digest(await readFile(prepared.sourcePath))).toBe(
      digest(prepared.sourceBytes),
    );
    await expect(access(prepared.destinationPath)).rejects.toBeDefined();
  ...
  it("records a single capability-disposition fault as truthful post-commit residue", async () => {
    ...
    const restore = setNativePublicationBindingForTests({
      ...
      publishNoReplace(...args) {
        ...
        renameSync(
          join(capability.path!, pngHandler.stagingFileName),
          destinationPath,
        );
        return "published";
      },
  ...
  it("sanitizes one PNG to two destinations concurrently with identical output", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "exifcleaner-qtx-png-concurrent-"),
    );
    directories.push(directory);
    const sourcePath = join(directory, "source.png");
    const sourceBytes = metadataPng();
    await writeFile(sourcePath, sourceBytes);
    const destinationPathA = join(directory, "destination-a.png");
    const destinationPathB = join(directory, "destination-b.png");

    const [resultA, resultB] = await Promise.all([
      sanitizeFile({
        sourcePath,
        destinationPath: destinationPathA,
        preserveOrientation: false,
        preserveColorProfile: false,
      ...
      sanitizeFile({
        sourcePath,
        destinationPath: destinationPathB,
        preserveOrientation: false,
        preserveColorProfile: false,
        preserveTimestamps: false,
        preserveResolution: false,
    ...
    const outputA = await readFile(destinationPathA);
    const outputB = await readFile(destinationPathB);
